...
raw_data <- ts(data = raw_data, start = c(2006,1), frequency = 4, names = c("gdp_pc","gov_gdp","gov_con_gdp","pub_inv_gdp","priv_inv_gdp","tr_op"))
ts_plot(raw_data)
...
seasonal_adj <- data.frame(Y=as.matrix(seasonal_adj),date=time(seasonal_adj))
